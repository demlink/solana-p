...
const KEYS_FOLDERR = path.resolve(__dirname, "../", "keys");

export const KEYS_FOLDER = KEYS_FOLDERR;
...
async function createDirectories(subDir?: string) {
  await fs.mkdirp(path.resolve(KEYS_FOLDERR));
  await fs.mkdirp(path.resolve(KEYS_FOLDERR, subDir || "persons"));
}

export const checkKeysDir = async () => fs.pathExists(KEYS_FOLDERR);

export const getPublicKey = async (name: string, type?: WhiteListKeyType) => {
  try {
    const publicKey = await fs.readJSON(
      path.resolve(KEYS_FOLDERR, type || "persons", name, "publicKey.json")
    );
...
export const storePublicKey = async (
  ...
  try {
    if (rewrite) {
      await fs.writeJSON(
        path.resolve(KEYS_FOLDERR, type, name, "publicKey.json"),
        publicKey.toString()
      );
    }
    return true;
...
export const storeKeypair = async (
  ...
  try {
    if (rewrite) {
      ...
      await fs.mkdirp(path.resolve(KEYS_FOLDERR, type, name));
      await fs.writeJSON(
        path.resolve(KEYS_FOLDERR, type, name, "privateKey.json"),
...
export const getMintToken = async (name: string, payer: Keypair) => {
  const mintPubKey = await fs.readJSON(
    path.resolve(KEYS_FOLDERR, "mints", name, "publicKey.json")
  );
  if (mintPubKey) {
    return new Token(
      SOLANA_CONNECTION,
...
export const checkIfKeyExists = async (
  ...
  try {
    return fs.pathExists(path.resolve(KEYS_FOLDERR, type, name));
  } catch (err) {
    return false;
  }
};
