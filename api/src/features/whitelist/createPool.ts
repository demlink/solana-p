...
(async function () {
  try {
    ...
    const poolRecipientTokenAccount = await getOrCreateAssociatedTokenAccount(
      SOLANA_CONNECTION,
      whitelistCreator,
    ...
    await storeTokenAccount(
      "swapProgramOwner",
      "poolRecipient",
      poolRecipientTokenAccount.address,
      true
    );
    console.log(poolRecipientTokenAccount.address.toString());

    const poolFeeTokenAccount = await getOrCreateAssociatedTokenAccount(
      SOLANA_CONNECTION,
      whitelistCreator,
      poolMintToken.publicKey,
      whitelistCreator.publicKey
    ...
    await storeTokenAccount(
      "swapProgramOwner",
      "poolFee",
      poolFeeTokenAccount.address,
      true
    );
    console.log(poolFeeTokenAccount.address.toString());
